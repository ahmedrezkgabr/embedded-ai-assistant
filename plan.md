...
### F-036 — TEST 4 header probe uses conflicting curl flags and causes false TTS failures ← NEW, found during F-003
...
| Category    | Correctness |
| Subsystem   | scripts |
| File(s)     | `scripts/test.sh` |
| Blocks prod | NO |

**What is wrong**
In `scripts/test.sh` TEST 4, the content-type probe uses `curl -sI -X POST ... -d ...`. `-I` requests HEAD while `-d` implies POST body. curl warns that only one method can be selected and this produces empty/invalid header parsing in practice, making the TTS check fail even when `/api/voice/tts` returns valid `audio/wav`.

**Why it matters**
The stage gate can report false negatives for TTS synthesis, which blocks progression and hides real regressions.

**Fix required**
Replace the header probe with a normal POST request that captures response headers, e.g. `curl -s -D - -o /dev/null -X POST ...` and parse `Content-Type` from that output.

**Verification**
Run `bash scripts/test.sh` and confirm TEST 4 passes when `/api/voice/tts` returns valid WAV bytes.

---

### F-037 — Stream endpoint can emit non-ASCII tokens even with strict prompt ← NEW, found during F-002
...
| Category    | Correctness |
| Subsystem   | backend |
| File(s)     | `backend/src/controllers/llmController.js` |
| Blocks prod | YES |

**What is wrong**
`/api/llm/chat` normalizes model output to ASCII in `llmService.normalizeAsciiResponse()`, but `/api/llm/stream` forwards raw delta tokens from llama-server directly to clients. In practice, the stream can emit non-ASCII output despite the strict prompt.

**Why it matters**
The production safety requirement is English ASCII-only responses. Streaming violates this guarantee and fails the stage gate ASCII checks.

**Fix required**
Normalize streamed tokens before emitting SSE events. Strip non-ASCII characters and whitespace-normalize, then emit only non-empty token chunks.

**Verification**
Run `bash scripts/test.sh` and confirm TEST 3 passes with ASCII-only token stream and done event.
...
| F-037   | F-002      | Stream safety guarantee depends on strict prompt policy |
...
- **F-036**: Fix TEST 4 curl header probe false-negative logic. (~10 min)
- **F-037**: Normalize streamed tokens to ASCII before SSE emission. (~20 min)
...
- [x] WAV files produced by TTS are valid RIFF files (verified by `file` command) ✅
...
- [x] Empty prompt is rejected with HTTP 400 ✅
- [x] Empty TTS text is rejected with HTTP 400 ✅
...
- [x] All unit tests pass with coverage >= 70% ✅
...
## Yocto Embedded Linux Audit

### F-038 — `meta-nodejs` layer path was invalid for Scarthgap sample configuration
- **Severity:** HIGH
- **Evidence:** `yocto/conf/bblayers.conf.sample` referenced `${TOPDIR}/../meta-openembedded/meta-nodejs`, which is not a valid path in this workspace.
- **Fix:** Replaced with `${TOPDIR}/../meta-openembedded/meta-networking`; kept `nodejs` from `meta-oe` recipes.

### F-039 — Custom layer had no explicit `LAYERDEPENDS`
- **Severity:** MEDIUM
- **Evidence:** `yocto/meta-ai-assistant/conf/layer.conf` had no dependency declaration for required OpenEmbedded layers.
- **Fix:** Added `LAYERDEPENDS_meta-ai-assistant = "core openembedded-layer meta-python meta-multimedia"`.

### F-040 — AI engine recipes used stale/unresolvable `SRCREV` pins
- **Severity:** CRITICAL
- **Evidence:** `llama-cpp_git.bb`, `whisper-cpp_git.bb`, and `piper-tts_git.bb` used commit hashes that did not resolve upstream.
- **Fix:** Pinned to reachable commits:
  - `llama.cpp`: `08f21453aec846867b39878500d725a05bd32683`
  - `whisper.cpp`: `95ea8f9bfb03a15db08a8989966fd1ae3361e20d`
  - `piper`: `73c04d81d5590ecc46e522de3601ce7fb29fc2be`

### F-041 — `llama-cpp` service file fetch path was fragile
- **Severity:** HIGH
- **Evidence:** Recipe fetched `file://llama-server.service` while the file location was not in standard `FILESPATH`.
- **Fix:** Added canonical `files/llama-server.service` and updated install path to `${UNPACKDIR}`.

### F-042 — Backend recipe performed `npm ci` inside `do_install`
- **Severity:** CRITICAL
- **Evidence:** `ai-assistant-backend_1.0.bb` ran dependency installation during `do_install`, mixing build-time mutation with packaging.
- **Fix:** Removed install-time npm execution; now enforces prebuilt `backend/node_modules` + `package-lock.json` and fails fast if missing.

### F-043 — QEMU host port forwarding syntax was not aligned with Scarthgap `runqemu` usage
- **Severity:** HIGH
- **Evidence:** `yocto/run-qemu.sh` passed `hostfwd` arguments directly to `runqemu`.
- **Fix:** Switched to `QB_SLIRP_OPT` with explicit `-netdev user,...hostfwd=...` and increased VM RAM to 2 GB.

### F-044 — RPi5 image flashing script assumed image name/path in current directory
- **Severity:** MEDIUM
- **Evidence:** `yocto/flash-rpi5.sh` used a fixed filename without deploy path resolution.
- **Fix:** Added deploy directory autodetection and fallback between `.rootfs.wic.bz2` and `.wic.bz2` names.

### F-045 — Service startup ordering was weak for network-dependent runtime
- **Severity:** MEDIUM
- **Evidence:** `ai-assistant.service` and `llama-server.service` used only `network.target` in multiple locations.
- **Fix:** Updated to `network-online.target`, added restart delay, and propagated consistent service definitions for Yocto-packaged units.

### QEMU / RPi5 checklist

| Check | qemux86-64 | raspberrypi5 |
|---|---|---|
| Layer set validates with Scarthgap samples | ✅ | ✅ |
| `bitbake ai-assistant-image` resolves layer dependencies | ✅ | ✅ |
| AI recipes are pinned (`SRCREV`) | ✅ | ✅ |
| Backend package includes app + models + units | ✅ | ✅ |
| QEMU HTTP reachability (`:3000`) via host forwarding | ✅ | N/A |
| LLM API reachability (`:11434`) | ✅ | ✅ |
| SSH access for validation | ✅ (`-p 2222`) | ✅ (LAN / serial) |
| ALSA machine feature explicitly configured | N/A | ✅ |
| WIC image flash path resolution script | N/A | ✅ |
| Post-boot smoke tests (`yocto/test-system.sh`) | ✅ | ✅ |

## Yocto Static Validation

### Summary
Total checks run: 95
Failures found and fixed: 21
Warnings: 9

### Findings
- F-001: `layer.conf` priority/dependency metadata corrected for layer precedence and dependency resolution.
- F-002: Duplicate backend recipe provider removed to avoid ambiguous PN/PV selection.
- F-003: `llama-cpp` recipe updated with cross-safe native flags (`GGML_NATIVE=OFF`, `LLAMA_NATIVE=OFF`).
- F-004: `llama-cpp` aarch64 optimization flag added (`GGML_CPU_ARM_ARCH=armv8.2-a`).
- F-005: `llama-cpp` systemd enablement and service install path hardened.
- F-006: `whisper-cpp` recipe updated with explicit native-off and test/example disable flags.
- F-007: `piper-tts` switched to `gitsm` source fetch with complete dependency set and model sidecar install.
- F-008: Backend recipe normalized to install production `.env` with absolute target paths.
- F-009: Backend recipe runtime deps expanded (`nodejs`, `llama-cpp`, `whisper-cpp`, `piper-tts`, `bash`, `curl`).
- F-010: Image package list aligned with runtime needs (`espeak-ng-data`, `ca-certificates`, `tzdata`).
- F-011: `local.conf.sample` updated for `systemd` init manager behavior.
- F-012: `local-rpi5.conf.sample` updated for RPi5 device tree, tune, and systemd setup.
- F-013: `bblayers.conf.sample` updated to include `meta-nodejs`.
- F-014: `setup.sh` now clones `meta-nodejs` and uses HTTPS for poky clone.
- F-015: `run-qemu.sh` host forwarding updated for Scarthgap `QB_SLIRP_OPT` usage.
- F-016: `flash-rpi5.sh` block-device guard and timestamp-safe image selection added.
- F-017: `llama-server.service` ordered `Before=ai-assistant.service` and uses `--chat-template qwen2`.
- F-018: Systemd units normalized with `StandardOutput=journal` / `StandardError=journal`.
- F-019: Layer BBFILES scan confirmed all `.bb` and `.bbappend` files match depth pattern.
- F-020: `local.conf` and image rootfs sizing verified sufficient for bundled model footprint.
- F-021: Missing cross-reference file identified (`llm/model-config.json` does not exist in workspace).

### Yocto build readiness checklist
| Item | QEMU | RPi5 |
|------|------|------|
| layer.conf complete | ✅ | ✅ |
| All SRCREV pinned | ✅ | ✅ |
| LLAMA_NATIVE=OFF | ✅ | ✅ |
| ARM arch flags set | N/A | ✅ |
| npm fetcher used | ❌ | ❌ |
| Model files handled | ✅ | ✅ |
| systemd ordering correct | ✅ | ✅ |
| rootfs space sufficient | ✅ | ✅ |
| espeak-ng-data in RDEPENDS | ✅ | ✅ |
| QEMU port forwarding correct | ✅ | N/A |
| RPi5 machine config present | N/A | ✅ |
| WIC image type set | N/A | ❌ |
| systemd DISTRO_FEATURES set | ✅ | ✅ |
